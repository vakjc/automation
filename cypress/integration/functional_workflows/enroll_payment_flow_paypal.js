describe('Enroll page payment flow without Upsell Paypal', function () {
  it('Enroll page payment flow without Upsell Paypal', function () {
    ...
    */
    ...
    var app_host_demo = 'eng-381.app.demo.jumpcut.com'
    var host_demo = 'eng-381.demo.jumpcut.com'
    var app_host = Cypress.env('app_host_demo')
    ...
    /*cy.get(".braintree-dispatch-frame").then($iframe => {
     ...
     //Document.getElementsByClassName('braintree-dispatch-frame').contentWindow.document.getElementById('#return_url').click()
     cy.window().then((win) => {
    ...
    cy.document().then((doc) => {
      //doc.find('.braintree-dispatch-frame')
      console.log(doc.getElementsByClassName("braintree-dispatch-frame")[0].id)
      cy.get('#' + doc.getElementsByClassName("braintree-dispatch-frame")[0].id)//('.braintree-dispatch-frame')
        .then(function ($iframe) {
          const $body = $iframe.contents().find('body');
          //console.log($body)
          const $doc = $iframe.contents().find('document');
          //getElementById('#return_url').click();
          $iframe.contents().find('a').click()
          cy.document().then((doc) => {
            console.log(doc)
          })
